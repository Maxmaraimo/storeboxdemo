        ...
        {tabs.map((tab) => (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all whitespace-nowrap cursor-pointer ${
              status === tab.id
                ? "bg-neutral-900 dark:bg-emerald-500/15 text-white dark:text-[#00d668] border border-transparent dark:border-emerald-500/30 shadow-xs font-black"
                ...
                <div className={`${isWideModal ? "lg:col-span-7" : ""} space-y-6`}>
                    ...
                    <div className="border border-slate-100 dark:border-white/5 rounded-2xl divide-y divide-slate-100 dark:divide-white/5 overflow-hidden">
                      {selectedOrder.items?.map((item) => (
                        ...
                        >
                          ...
                          <div className="font-mono font-black text-slate-900 dark:text-emerald-400 text-sm whitespace-nowrap">
                  ...
                  <div className="p-4 sm:p-5 rounded-2xl bg-slate-50 dark:bg-zinc-900 border border-slate-200/80 dark:border-white/10 space-y-2.5 font-mono text-xs">
                    ...
                    <div className="pt-2 flex flex-wrap items-center justify-between gap-2 text-[11px] font-sans border-t border-slate-200/60 dark:border-white/5 text-slate-500">
                      <span>To`lov usuli: <b>{selectedOrder.payment_method_display || "Naqd pul"}</b></span>
                      <span>Holati: <b className="text-slate-800 dark:text-slate-200">{selectedOrder.payment_status_display || "Kutilmoqda"}</b></span>
                    </div>
                  </div>
                </div>
                ...
                <div className={`${isWideModal ? "lg:col-span-5" : ""} space-y-5`}>
                  ...
                  <div className="p-4 rounded-2xl bg-slate-50 dark:bg-zinc-900/60 border border-slate-200/80 dark:border-white/10 space-y-2.5">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      >
                        <Clock className="w-3.5 h-3.5" /> Jarayonga
                      </button>
                      <button
                        type="button"
                      ...
                      >
                        <Truck className="w-3.5 h-3.5" /> Kuryerda
                      </button>
                      <button
                        type="button"
                        onClick={() => updateStatusMutation.mutate({ orderId: selectedOrder.id, newStatus: "COMPLETED" })}
                        className="py-2.5 px-3 rounded-xl bg-emerald-50 text-emerald-700 hover:bg-emerald-100 font-bold text-xs transition-colors flex items-center justify-center gap-1.5"
                      >
                      ...
                      >
                        <XCircle className="w-3.5 h-3.5" /> Bekor qilish (Qoldiqni qaytarish)
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="p-4 rounded-2xl bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-white/10 space-y-2">
                    ...
                    <div className="flex items-center gap-2 pt-1">
                      <a
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-emerald-50 text-slate-700 hover:text-emerald-700 font-mono text-xs font-bold transition-colors"
                      >
                  ...
                  <div className="p-4 rounded-2xl bg-white dark:bg-zinc-900 border border-slate-200/80 dark:border-white/10 space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="text-[11px] font-black text-slate-400 uppercase tracking-wider flex items-center gap-1">
                        <MapPin className="w-3.5 h-3.5 text-rose-500" /> Yetkazib berish manzili
                      </div>
                    ...
                    <div className="text-xs font-medium text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-zinc-800/60 p-3 rounded-xl border border-slate-200/60 dark:border-white/5">
                      {selectedOrder.delivery_address || "Manzil ko`rsatilmagan"}
                    </div>

                    {/* Interactive External Map Links */}
                    {selectedOrder.delivery_lat && selectedOrder.delivery_lng ? (
                      <div className="space-y-2.5">
                        <div className="flex items-center gap-2">
                          <a
                            href={`https://yandex.uz/maps/?pt=${selectedOrder.delivery_lng},${selectedOrder.delivery_lat}&z=16&l=map`}
                            target="_blank"
                            rel="noreferrer"
                            className="flex-1 py-2 px-3 rounded-xl bg-amber-50 hover:bg-amber-100 text-amber-900 text-xs font-bold flex items-center justify-center gap-1.5 transition-colors"
                          >
                            <ExternalLink className="w-3.5 h-3.5 text-amber-600" /> Yandex Xarita
                          </a>
                          <a
                            href={`https://www.google.com/maps?q=${selectedOrder.delivery_lat},${selectedOrder.delivery_lng}`}
                            target="_blank"
                            rel="noreferrer"
                            className="flex-1 py-2 px-3 rounded-xl bg-blue-50 hover:bg-blue-100 text-blue-900 text-xs font-bold flex items-center justify-center gap-1.5 transition-colors"
                          >
                            <ExternalLink className="w-3.5 h-3.5 text-blue-600" /> Google Maps
                          </a>
                        </div>
                        {/* Map Preview Frame */}
                        <div className="w-full h-44 rounded-xl overflow-hidden border border-slate-200 dark:border-white/10 relative">
                          <iframe
                            ...
                            style={{ border: 0 }}
                            loading="lazy"
                            src={`https://www.openstreetmap.org/export/embed.html?bbox=${Number(selectedOrder.delivery_lng) - 0.005}%2C${Number(selectedOrder.delivery_lat) - 0.005}%2C${Number(selectedOrder.delivery_lng) + 0.005}%2C${Number(selectedOrder.delivery_lat) + 0.005}&layer=mapnik&marker=${selectedOrder.delivery_lat}%2C${selectedOrder.delivery_lng}`}
                          />
                        </div>
                      </div>
                    ) : (
                      <div className="text-xs text-slate-400 italic bg-slate-50 dark:bg-zinc-800/40 p-2.5 rounded-xl text-center">
                        Geolokatsiya koordinatalari mavjud emas
                      </div>
                    )}
                  </div>
                </div>
